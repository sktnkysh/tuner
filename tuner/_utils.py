...
def load_fromdir(dir_name, label2id=None, resize=RESIZE):
...
def load_fromdf(dataframe, label2id=None, resize=RESIZE):
    df = dataframe

    labels = list(set(df['label'].drop_duplicates().values))
    l2i = label2id if label2id else {label: i for i, label in enumerate(labels)}
